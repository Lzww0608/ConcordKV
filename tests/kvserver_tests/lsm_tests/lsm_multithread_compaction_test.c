/*
 * @Author: Lzww0608  
 * @Date: 2025-6-5 20:59:49
 * @LastEditors: Lzww0608
 * @LastEditTime: 2025-6-5 20:59:50
...
static lsm_memtable_manager_t* create_test_memtable_manager(void) {
    ...
    return lsm_memtable_manager_create(&config, "test_manager");
}
...
static bool test_scheduler_basic_multithread(test_result_t *result) {
    ...
    // 创建压缩调度器
    lsm_compaction_scheduler_t *scheduler = create_test_scheduler(memtable_manager, level_manager);
    if (!scheduler) {
...
static bool test_concurrent_task_submission(test_result_t *result) {
    ...
    for (int i = 0; i < task_count; i++) {
        ...
        // 创建虚拟的MemTable用于测试（空MemTable）
        char memtable_name[64];
        snprintf(memtable_name, sizeof(memtable_name), "test_memtable_%d", i);
        lsm_memtable_t *test_memtable = lsm_memtable_create(1024, memtable_name);
        if (!test_memtable) {
            lsm_compaction_task_destroy(tasks[i]);
            snprintf(result->details, sizeof(result->details), 
                    "无法创建测试MemTable %d", i);
            goto cleanup;
        }
        ...
        // 设置Level-0压缩参数
        int ret = lsm_compaction_task_set_level0_params(tasks[i], test_memtable, 0);
        if (ret != KV_ERR_NONE) {
            lsm_memtable_destroy(test_memtable);
            lsm_compaction_task_destroy(tasks[i]);
            snprintf(result->details, sizeof(result->details), 
                    "无法设置任务参数 %d: %d", i, ret);
            goto cleanup;
...
static bool test_task_priority_scheduling(test_result_t *result) {
    ...
    lsm_compaction_task_t *low_task = lsm_compaction_task_create(
        LSM_COMPACTION_TASK_LEVEL0, LSM_COMPACTION_PRIORITY_LOW);
    ...
    // 为每个任务创建虚拟MemTable
    lsm_memtable_t *memtables[3];
    for (int i = 0; i < 3; i++) {
        char memtable_name[64];
        snprintf(memtable_name, sizeof(memtable_name), "priority_memtable_%d", i);
        memtables[i] = lsm_memtable_create(1024, memtable_name);
        if (!memtables[i]) {
            snprintf(result->details, sizeof(result->details), "无法创建测试MemTable");
            goto cleanup;
        }
    }
    
    // 设置任务参数
    lsm_compaction_task_set_level0_params(low_task, memtables[0], 0);
    lsm_compaction_task_set_level0_params(high_task, memtables[1], 0);
    lsm_compaction_task_set_level0_params(urgent_task, memtables[2], 0);
