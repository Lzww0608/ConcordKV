...
type RoutingResult struct {
	...
	CacheTime     time.Time       `json:"cacheTime"`     // 缓存时间戳
}
...
func (sr *SmartRouter) Route(req *RoutingRequest) (*RoutingResult, error) {
	...
	// 获取分片信息
	shardInfo, found := sr.topologyCache.GetByKey(req.Key)
	if !found || shardInfo == nil {
		atomic.AddInt64(&sr.stats.FailedRequests, 1)
		return nil, fmt.Errorf("获取分片信息失败: 未找到键 %s 对应的分片", req.Key)
	}
...
func (sr *SmartRouter) getFromCache(key string) (*RoutingResult, bool) {
	...
	// 检查TTL
	if time.Since(result.CacheTime) > sr.config.CacheTTL {
		return nil, false
	}
...
func (sr *SmartRouter) addToCache(key string, result *RoutingResult) {
	...
	// 添加到缓存
	resultCopy := *result
	resultCopy.CacheTime = time.Now() // 记录缓存时间
