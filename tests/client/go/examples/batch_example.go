...
* @Description: ConcordKV Go client - batch_example.go
 */
package main
...
// 批量设置示例
func batchSetExample(client *mockpkg.Client) {
	fmt.Println("\n--- 批量设置示例 ---")
...
// 批量获取示例
func batchGetExample(client *mockpkg.Client) {
	fmt.Println("\n--- 批量获取示例 ---")
...
// 批量删除示例
func batchDeleteExample(client *mockpkg.Client) {
	fmt.Println("\n--- 批量删除示例 ---")
...
// 事务内批量操作示例
func transactionBatchExample(client *mockpkg.Client) {
	fmt.Println("\n--- 事务内批量操作示例 ---")

	// 创建事务
	tx := client.NewTransaction().WithIsolation(mockpkg.IsolationSerializable)
